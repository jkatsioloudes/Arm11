...
/* Sets the bits as specified of a 32-bit word from startBit to endBit*/
uint32_t setBits(uint32_t instructionWord, uint32_t startBit, uint32_t endBit,
		uint32_t value) {
	instructionWord &= ~createMask(startBit, endBit);
	value <<= startBit;
	/* Mask off extra bits*/
	value &= createMask(startBit,endBit);
	instructionWord |= value;
	return instructionWord;
}

/* Reads a 32-bit word (4 bytes) from a memory address */
uint32_t readWord(uint8_t *memory) {
	return (memory[0] | (memory[1] << (BITS_IN_BYTE))
			| (memory[2] << (BITS_IN_BYTE * 2)) | (memory[3] << (BITS_IN_BYTE * 3)));
}
...
/* Changes the endianness of a 32-bit word (big->little , little->big */
uint32_t changeEndianness(uint32_t word) {
